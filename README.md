<h1 align="center">Hi 👋, I'm Vishal M Bharadwaj</h1>

<p align="center">
Full Stack Developer specializing in JavaScript, React, Node.js, and Express, with experience building scalable, production-ready web applications. Passionate about creating clean user experiences, designing maintainable system architectures, and developing secure, high-performance applications from frontend to backend.
</p>

---

## 🚀 Tech Stack

<p align="center">
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" width="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original-wordmark.svg" width="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original-wordmark.svg" width="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/express/express-original-wordmark.svg" width="40"/>
<img src="https://cdn.worldvectorlogo.com/logos/nextjs-2.svg" width="40"/>

<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" width="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg" width="40"/>
<img src="https://www.vectorlogo.zone/logos/sqlite/sqlite-icon.svg" width="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg" width="40"/>
<img src="https://www.vectorlogo.zone/logos/tailwindcss/tailwindcss-icon.svg" width="40"/>
<img src="https://www.vectorlogo.zone/logos/git-scm/git-scm-icon.svg" width="40"/>
<img src="https://www.vectorlogo.zone/logos/getpostman/getpostman-icon.svg" width="40"/>
</p>

---
## 🎯 Core Expertise

- Full Stack Web Development
- Frontend Architecture (React)
- REST API Development
- Authentication & Authorization (JWT)
- Responsive UI Development
- Database Design (MySQL, SQLite, MongoDB)
- Component-Based Design
- Performance & Maintainability

---
## 💼 Featured Projects

### 🤝 SynsSpace
- Real-time collaborative workspace built with React, Node.js, Express, MySQL, and Socket.IO
- Secure JWT authentication with modular client-server architecture
- Team chat, task management, notes, file sharing, and collaborative productivity features

🔗 https://github.com/Vishalbharadwaj27/SynsSpace

---

### 📊 One-App Premium Dashboard
- Full-stack productivity dashboard with secure JWT authentication and modular widget architecture
- Built using React, Express, SQLite, Tailwind CSS, and Context API
- Combines tasks, notes, reminders, expenses, bookmarks, and productivity tools into one application

🔗 https://github.com/Vishalbharadwaj27/One-App-Premium-Dashboard

---

### 🩺 Doc Assist
- Full-stack healthcare management application for patient records, appointments, and medical notes
- Responsive React frontend with Express backend and PDF report generation
- Designed with scalable architecture, offline support, and production-ready project structure

🔗 https://github.com/Vishalbharadwaj27/Doc
---


## 💡 Currently Working On

- Building production-ready full-stack applications
- Learning Next.js and advanced React patterns
- Strengthening backend architecture and system design
- Exploring AI-powered features for modern web applications
---

## 🎯 Open To Opportunities

- Software Engineer (Full Stack) roles  
- Internship / Entry-level positions  
- Open to collaboration on real-world projects  

---

## 🌐 Connect With Me

<p align="center">
<a href="https://www.linkedin.com/in/vishal-m-bharadwaj-059b5a353/">
<img src="https://img.shields.io/badge/LinkedIn-Vishal%20Bharadwaj-blue?style=for-the-badge&logo=linkedin"/>
</a>

<a href="https://vishal-m-bharadwaj-portfolio.vercel.app/">
<img src="https://img.shields.io/badge/Portfolio-Visit%20My%20Work-black?style=for-the-badge&logo=vercel"/>
</a>
</p>




